  ...
  return (
    <section className="relative overflow-hidden bg-gradient-luxe py-32">
      ...
      <div className="relative mt-16">
        <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-16 sm:w-32 bg-gradient-to-r from-[color:var(--background)] to-transparent" />
        <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-16 sm:w-32 bg-gradient-to-l from-[color:var(--background)] to-transparent" />
        <div className="flex w-max animate-marquee gap-6 px-6">
          {loop.map((r, i) => (
            <motion.figure
              key={i}
              whileHover={{ y: -6 }}
              className="w-[290px] sm:w-[360px] shrink-0 rounded-3xl glass p-6 sm:p-7 shadow-soft text-foreground"
            >
              <Quote className="h-6 w-6 sm:h-7 sm:w-7 text-primary" />
              <blockquote className="mt-4 font-display text-base sm:text-lg leading-snug text-foreground/90">
                "{r.text}"
              </blockquote>
              <figcaption className="mt-6 flex items-center gap-3">
                <span className="grid h-10 w-10 sm:h-11 sm:w-11 shrink-0 place-items-center rounded-full bg-gradient-rose text-sm font-medium text-ivory">
                  {r.initials}
                </span>
                <div>
                  <div className="text-sm font-medium">{r.name}</div>
                  <div className="text-[10px] sm:text-[11px] uppercase tracking-[0.25em] text-muted-foreground">{r.city}</div>
                </div>
              </figcaption>
            </motion.figure>
          ))}
